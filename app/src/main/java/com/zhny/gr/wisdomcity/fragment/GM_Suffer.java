...
public class GM_Suffer extends BaseFragment_v4 {

    private volatile static GM_Suffer leaderReferral;

    public GM_Suffer() {
    }

    public static GM_Suffer getInstance() {
        if (leaderReferral == null) {
            synchronized (GM_Suffer.class) {
                if (leaderReferral == null) {
                    leaderReferral = new GM_Suffer();
                }
            }
        }
