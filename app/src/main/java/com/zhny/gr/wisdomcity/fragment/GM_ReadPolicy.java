...
public class GM_ReadPolicy extends BaseFragment_v4 {

    private volatile static GM_ReadPolicy leaderReferral;

    public GM_ReadPolicy() {
    }

    public static GM_ReadPolicy getInstance() {
        if (leaderReferral == null) {
            synchronized (GM_ReadPolicy.class) {
                if (leaderReferral == null) {
                    leaderReferral = new GM_ReadPolicy();
                }
            }
        }
