...
public class GM_Measure extends BaseFragment_v4 {

    private volatile static GM_Measure leaderReferral;

    public GM_Measure() {
    }

    public static GM_Measure getInstance() {
        if (leaderReferral == null) {
            synchronized (GM_Measure.class) {
                if (leaderReferral == null) {
                    leaderReferral = new GM_Measure();
                }
            }
        }
